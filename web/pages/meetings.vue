<template>
  <div>
      ...
      <UCard>
        <UForm class="space-y-4" @submit="submitForm">
          ...
          <UFormGroup for="module_id" label="Module:">
            <USelect v-model="module_id" :options="moduleOptions" option-attribute="name" value-attribute="id" id="module_id" />
          </UFormGroup>
    ...
    <UCard v-for="meeting in meetings" :key="meeting.id" title="Meeting Information">
      ...
      <p><strong>Module ID:</strong> {{ getModuleName(meeting.module_id) }}</p>
    </UCard>
  </div>
</template>
...
const { meetings, loading, error, addMeeting } = meetingStore
...
const moduleOptions = computed(() => {
  if(!modules) return []
  return modules.map(module => ({ name: module.name, id: module.id }))
})
